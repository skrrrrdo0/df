  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (key === 'ArrowUp' || key === 'w') {
      ...
      newX += 1;
       ...
       // 새로운 코드: 중력 센서로 공 제어하기
       window.addEventListener('deviceorientation', (event) => {
        const tiltX = event.beta; // 기울기 X (앞뒤)
        const tiltY = event.gamma; // 기울기 Y (좌우)

        ballPosition.x += tiltY * speed * 0.1; // 기울기 값을 조정해 속도 조절
        ...
        setBallPosition(ballPosition.x / 8, ballPosition.y / 8); // 위치 조정
    });
